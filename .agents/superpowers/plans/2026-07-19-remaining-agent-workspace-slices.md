...
| --- | --- | --- | --- |
| `adventures-project-doctrine` | A repo-backed task needs a project invariant before implementation. | The no-skill agent routed through the retired `Patch / project-director` identity and old `Patch/` governance files. | Thin router into `.agents/doctrine/`; frontmatter, route table, composition contract, and character-only boundary were checked. |
