...
The repository runs `.github/workflows/ci.yml` on non-draft pull requests and `main` pushes. The job executes the same preflight as the pre-commit hook: `python tools/run.py ci --check`.
