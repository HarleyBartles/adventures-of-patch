...
DEFAULT_LOCAL_SKILL_PREFIXES = ("adventures-",)
...
class MarketplaceManifest:
    ...
    local_skill_prefixes: tuple[str, ...]
...
def load_manifest_data(data: dict) -> MarketplaceManifest:
    ...
    raw_local_skill_prefixes = data.get("local_skill_prefixes", list(DEFAULT_LOCAL_SKILL_PREFIXES))
    if not isinstance(raw_local_skill_prefixes, list) or not all(
        isinstance(prefix, str) and prefix for prefix in raw_local_skill_prefixes
    ):
        raise ValueError("Marketplace manifest local_skill_prefixes entry must be a non-empty string list")
    ...
    return MarketplaceManifest(
        ...
        local_skill_prefixes=tuple(raw_local_skill_prefixes),
...
def is_local_skill_name(name: str, prefixes: tuple[str, ...]) -> bool:
    ...
    return any(name_key.startswith(prefix.casefold()) for prefix in prefixes)


def tracked_local_skill_names(output_root: Path, prefixes: tuple[str, ...]) -> set[str]:
    """Return tracked repository-owned skill directories below the output root."""
    ...
    if result.returncode != 0:
        raise RuntimeError(
            "git ls-files failed while checking repository local skill custody:\n"
    ...
    for raw_path in result.stdout.decode("utf-8", errors="replace").split("\0"):
        ...
        try:
            ...
            continue
        if skill_path.parts and is_local_skill_name(skill_path.parts[0], prefixes):
            names.add(skill_path.parts[0])
    return names


def local_skill_names(output_root: Path, prefixes: tuple[str, ...]) -> set[str]:
    """Return repository-owned skill directories that refresh must preserve."""
    if not output_root.exists():
        return set()

    names: set[str] = set()
    for path in output_root.iterdir():
        if not is_local_skill_name(path.name, prefixes):
            continue
        if not path.is_dir():
            raise ValueError(
                f"Repository local skill must be a directory: {path.name}"
            )
        names.add(path.name)
    if not names:
        return set()
    tracked_names = tracked_local_skill_names(output_root, prefixes)
    ...
    if untracked:
        raise ValueError(
            "Repository local skills must be tracked before refresh: "
            + ", ".join(untracked)
        )
    return names
...
def sync_default_skills(
    ...
    *,
    ...
    preserved_local_skill_names = local_skill_names(output_root, manifest.local_skill_prefixes)
    ...
    for plugin in expected_plugins:
        ...
        for skill_name, skill_source in read_skill_directories(plugin, source_root):
            ...
            if is_local_skill_name(skill_name, manifest.local_skill_prefixes):
                continue
            eligible_skill_count += 1
            if normalized_skill_name in preserved_local_skill_keys:
                raise ValueError(
                    "Marketplace skill collides with a repository local skill: "
                    f"{skill_name}"
                )
            ...
            existing = skill_records.get(normalized_skill_name)
            if existing is None:
                skill_records[normalized_skill_name] = {
                    "name": skill_name,
                    "source": skill_source,
            ...
            if not isinstance(existing_source, Path) or not trees_match(existing_source, skill_source):
                first_provider = existing["providers"][0]
                raise ValueError(
                    "Marketplace skill name has conflicting provider content: "
                    f"{skill_name} ({first_provider}, {plugin.name})"
                )
            existing["providers"].append(plugin.name)
        if eligible_skill_count == 0:
            raise ValueError(
                f"Configured marketplace plugin {plugin.name!r} has no eligible skills after local-prefix filtering; "
                "remove it from default_plugins when the marketplace removes the provider."
            )

    expected_skill_names = [record["name"] for record in skill_records.values()]
    desired_skill_dirs = {
        record["name"]: record["source"] for record in skill_records.values()
    }
    skill_providers = {
        record["name"]: record["providers"] for record in skill_records.values()
    }
    expected_root_names = (
    ...
    provenance_data = {
        ...
        "local_skill_prefixes": list(manifest.local_skill_prefixes),
