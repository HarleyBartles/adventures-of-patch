# Incoming asset zip ingress lane

Purpose of this folder:

- Canonical local drop zone for incoming asset zip packages.
- Evidence/input staging area only; incoming zips are not canon.
- Temporary holding area while a repo worker inspects, classifies, and lands repo files.

Harley drop instruction:

- Place approved incoming asset zip packages directly in `assets/source-zips/incoming/` unless a future dispatch names a specific subfolder.
...
- The worker deletes successfully ingressed zips after the landed repo files are committed, pushed on a worker branch, and PR/merge-verified.
